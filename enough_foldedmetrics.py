...
# normal matrix
def getMetrics(test_df, y_true, y_pred, biased_col, samples, total_pts, fold, enough_multiplier):
    ...
    return [recall, precision, accuracy, F1, AOD, EOD, SPD, FA0, FA1, biased_col, samples, total_pts, fold, enough_multiplier]
...
def sampleMetrics(test_df, y_true, y_pred, biased_cols, samples, total_pts, f, enough_multiplier):
    colmetrics = {}

    for i in biased_cols:
        # print(test_df, y_true, y_pred, i, samples, run_num)
        colmetrics[i] = getMetrics(test_df, y_true, y_pred, i, samples, total_pts, f , enough_multiplier)
    return colmetrics
...
def main():
    ...
    for dataset in pbar:
        ...
        for s in treatments:
            ...
            for f in range(1,26):
                ...
                for m in mtreatments:
                    dfm = copy.deepcopy(dfr)
                    dfm.drop(dfm.loc[dfm['enough_multiplier']!= m].index, inplace=True)

                    if '!probability' in dfm.columns:
                        y_true = dfm["!probability"]
                    else:
                        y_true = dfm["!Probability"]
                    y_pred = dfm["predicted"]
                    # print(dfr)
                    tp = dfm["total_pts"].tolist()
                    list.insert(f, sampleMetrics(dfm, y_true, y_pred, biased_cols, s, tp[0], f, m))
            all_metrics[s] = list
        ...
        for f in range(1,26):
            ...
            list2.insert(f, sampleMetrics(dfr2, y_true, y_pred, biased_cols, smaller, tp[0], f, 100))
