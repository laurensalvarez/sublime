...
def clusterandclassify(table, filename):
    ...
    ptable = Table(100)
    pheader = deepcopy(table.header)
    ptable + pheader
    for row in trows:
        ptable + row

    pertubeddf = pd.DataFrame(columns=pheader)

    for col in ptable.cols:
        random.shuffle(col.vals)
        pertubeddf[col.name] = col.vals

    rkf = RepeatedKFold(n_splits=5, n_repeats=5, random_state=222)

    OGmarker = [0 for _ in range(len(trows))]
    pmarker = [1 for _ in range(len(trows))]
    yvals = []
    yvals.extend(OGmarker)
    yvals.extend(pmarker)

    # print(dsdf.shape)
    dsdf = dsdf.append(pertubeddf)
    ...
    tcols = list(dsdf.columns.values)
    # tcols.append("yvals")
    ...
    for train_index, test_index in rkf.split(dsdf,y):
        ...
        y_train = dsdf.iloc[train_index]['yvals']
        y_test = dsdf.iloc[test_index]['yvals']
    ...
    final_columns = []
    for col in table2.protected:
        final_columns.append(col.name)
    final_columns.append("yvals")
    ...
    final_df.to_csv("./output/newDS/" + filename + "_LR.csv", index=False)
...
def main():
    random.seed(10019)
    datasets = ["defaultcredit.csv", "bankmarketing.csv"] #"diabetes.csv", , "CleanCOMPAS53.csv"
    pbar = tqdm(datasets)
